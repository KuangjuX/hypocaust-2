pub mod stack {
    ...
    use crate::{
        constants::{layout::TRAP_CONTEXT, KERNEL_STACK_SIZE, PAGE_SIZE},
        mm::MapPermission,
    };
    ...
    pub fn hstack_alloc(guest_id: usize) -> HypervisorStack {
        ...
        info!(
            "Allocated hstack: [{:#x}: {:#x})",
            hstack_bottom, hstack_top
        );
        ...
        pub fn parse(dtb: usize) -> Self {
            ...
            for node in fdt.find_all_nodes("/soc/virtio_mmio") {
                if let Some(reg) = node.reg().and_then(|mut reg| reg.next()) {
                    ...
                    info!("virtio mmio addr: {:#x}, size: {:#x}", paddr, size);
            ...
            for node in fdt.find_all_nodes("/soc/test") {
                if let Some(reg) = node.reg().and_then(|mut reg| reg.next()) {
                    ...
                    let size = reg.size.unwrap();
                    info!("test addr: {:#x}, size: {:#x}", base_addr, size);
            ...
            for node in fdt.find_all_nodes("/soc/uart") {
                if let Some(reg) = node.reg().and_then(|mut reg| reg.next()) {
                    let base_addr = reg.starting_address as usize;
                    let size = reg.size.unwrap();
                    info!("UART addr: {:#x}, size: {:#x}", base_addr, size);
            ...
            for node in fdt.find_all_nodes("/soc/clint") {
                if let Some(reg) = node.reg().and_then(|mut reg| reg.next()) {
                    let base_addr = reg.starting_address as usize;
                    let size = reg.size.unwrap();
                    info!("CLINT addr: {:#x}, size: {:#x}", base_addr, size);
            ...
            for node in fdt.find_all_nodes("/soc/plic") {
                if let Some(reg) = node.reg().and_then(|mut reg| reg.next()) {
                    let base_addr = reg.starting_address as usize;
                    let size = reg.size.unwrap();
                    info!("PLIC addr: {:#x}, size: {:#x}", base_addr, size);
            ...
            for node in fdt.find_all_nodes("/soc/pci") {
                if let Some(reg) = node.reg().and_then(|mut reg| reg.next()) {
                    let base_addr = reg.starting_address as usize;
                    let size = reg.size.unwrap();
                    info!("PCI addr: {:#x}, size: {:#x}", base_addr, size);
...
use crate::constants::csr::{hcounteren, hedeleg, hideleg};
...
pub unsafe fn init_vmm(hpm: HostMemorySet<PageTableSv39>) {
    ...
    hideleg::write(hideleg::VSEIP | hideleg::VSSIP | hideleg::VSTIP);
    ...
    info!("Initialize hypervisor environment");
}
